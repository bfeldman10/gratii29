...
// *************** START BLACKJACK GAME LOGIC *************************
	...
	function addBet(amt) {
		...
		if (myGratii >= amt) {
			...
			//alert("POSTED");
		} else { alert("Not enough Gratii bro"); }
	}
	...
	if (gameState != 'split') {
		...
		if (winstate == "playerblackjack") {
			gratiiWinnings += Math.round(((3/2) * betTotal) + betTotal);
			myGratii += Math.round(((3/2) * betTotal) + betTotal);
			totalWon += Math.round((3/2) * betTotal);
			//previousBet = betTotal;
			//betTotal = 0;
			gameState = 'game_over';
			//alert("BLACKJACK!!");
		...
		temporaryWon = 0;
		if (winstate == "dealer") {
			//alert("Dealer wins this hand. You lost " + betTotal/2 + " Gratii");
			//declare total won
			temporaryWon -= Math.round(betTotal/2);
			//console.log("PAYWINNINGS / dealer win: temporaryWon -= Math.round(betTotal/2)");
			totalWon -= betTotal/2; //ADDEDNEW
			//reset previous bet and betTotal
			//previousBet = betTotal;
			//update game and win states
			//gameState = 'game_over';
		...
		if (winstate == "player") {
			//alert("Player wins this hand. You won " + betTotal/2 + " Gratii");
			//myGratii += betTotal;
			totalWon += betTotal; //ADDEDNEW
			
			temporaryWon += Math.round(betTotal);
			//console.log("PAYWINNINGS / player win: temporaryWon -= Math.round(betTotal/2)");
			splitCurrentHand = 2;
			//return;
		}		
		if (winstate == "tie") {
			...
			//myGratii += betTotal/2;
			temporaryWon += Math.round(betTotal/2);
			//console.log("PAYWINNINGS / tie: temporaryWon -= Math.round(betTotal/4)");
			splitCurrentHand = 2;
			//return;
		}		
		if (winstate == "playerblackjack") { //split 'blackjack' doesn't pay 3/2 odds, pays out normally.
			//alert("Player wins this hand. You won " + betTotal/2 + " Gratii");
			//myGratii += betTotal;
			totalWon += betTotal/2; //ADDEDNEW
			temporaryWon += Math.round(betTotal);
			//console.log("PAYWINNINGS / playerblackjack: temporaryWon -= Math.round(betTotal/2)");
			splitCurrentHand = 2;
			//return;
		}			
		...
		if (temporaryWon > 0) {
			//alert("Player wins first hand. You won " + temporaryWon + " Gratii");
			//totalWon += temporaryWon;
			//totalWon += betTotal/2; //ADDEDNEW
			...
			//alert("Dealer wins first hand. You lost " + (-1)*temporaryWon + " Gratii");
			//totalWon -= temporaryWon;
			//totalWon -= betTotal/2; //ADDEDNEW
			//temporaryWon = 0;
		} else if (temporaryWon == 0) {
			//alert("First Hand: Tie Game.");
		}
		...
		if (winstate == "dealer") {
			...
			//reset previous bet and betTotal
			//previousBet = Math.round(betTotal/2);
			//betTotal = 0;
			//update game and win states
			//alert("Dealer wins second hand. You lost " + betTotal/2 + " Gratii");
		...
		if (winstate == "player") {
			//alert("Player wins second hand. You won " + betTotal/2 + " Gratii");
			gratiiWinnings += Math.round(betTotal);
			myGratii += Math.round(betTotal);
			//postGratiiWon(gratiiWinnings);
			//getUserGratii();	
			totalWon += Math.round(betTotal/2); //ADDEDNEW
			//reset previous bet and betTotal
			//previousBet = Math.round(betTotal/2);
			//betTotal = 0;
			//update game and win states
	...
	//if (gratiiWinnings > 0) {
		//myGratii += gratiiWinnings; //to replace getUserGratii()
		//console.log("Paywinnings gratii");
		postGratiiWon(gratiiWinnings);
		gratiiWinnings = 0;
		//getUserGratii();
	//}
...
// *************** START USER INTERFACE LOGIC *************************
	...
	*/
	...
	function drawStatusBar() {
		...
		draw.text('Current Bet:', betIndicatorsLeft, 30, 12, '#cbcbcb');
			draw.text(previousBet, betIndicatorsDisplayLeft, 30, 12, '#cbcbcb');
	...
	//if (betTotal > 0 && gameState == 'bet') {
		...
		if (gameState != 'split' && gameState != 'split_game_over') {
			if (winnerState == 'player') {
				...
				draw.text('+' + Math.round((3/2)*betTotal), resultX, canvas.height/2-20, 24, '#02e807');
				canvas.getContext("2d").drawImage(gPlusImage, iconsX-10, canvas.height/2-40);
			}
