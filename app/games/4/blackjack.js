...
/***********************************************BDF API***********************************************/
	window.eventHandler = {
		...
		submitBet: function(bet, betType){
			...
			var jsonString = JSON.stringify(parent.user.arcadeEvents);
			...
			$.ajax({
		        ...
		        timeout: 30000,
		        error: function(data){
		        	console.log( 'ERROR submitting bet in balck jack' );
		        	console.log(data);
		...
		submitWinLose: function(amount, winnerState, gameState, finalHand){
			...
			var thisGameID = parent.user.gameInProgress['gameID'];
			var equations = parent.user.gameInProgress['equations'];
			var thisGameEvent = resultEvent;
			var equationForThisEvent = equations[thisGameEvent];
			var scoreForThisEvent = amount;
			var gratiiEarned = Math.floor(scoreForThisEvent*equationForThisEvent);
			console.log(gratiiEarned);
			...
			parent.user.changeGratii(amount);

			if (finalHand) {
				console.log(parent.user.arcadeEvents);
...
/***********************************************BDF API***********************************************/
	...
	function postGratiiWon() {
		...
		window.eventHandler.submitWinLose(gratiiWinnings, winnerState, gameState, true);
	...
	function postGratiiWonDeckOne(temporaryWon) {
		...
		// game state normal is: "game_over"
		// game state on a split is "split_game_over"
		window.eventHandler.submitWinLose(temporaryWon, winnerState, gameState, false);
		return;
	}
...
function payWinnings(winstate) {
	...
	window.finishDoubleDown = function(){
		//getUserGratii();
	...
	// Call back when successfull bet placed on back-end API
	window.finishSplit = function(){
		//getUserGratii();
		myGratii -= betTotal
		betTotal *= 2;
